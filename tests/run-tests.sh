...
setup_test_repo main-branch git@github.com:gilessmart/giturl.git main $clear_repos
setup_test_repo slash-branch git@github.com:gilessmart/giturl.git 'test-branches/abc' $clear_repos
setup_test_repo special-chars-branch git@github.com:gilessmart/giturl.git 'test-branches/_=+,.@£' $clear_repos
printf '\n'
...
expected="https://github.com/gilessmart/giturl/blob/$main_branch_hash"
...
expected="https://github.com/gilessmart/giturl/blob/$main_branch_hash/README.md"
test "$description" "$command" "$expected" || ((++failed_tests))

description="With nested folder path"
command="'$GITURL_PATH' '$TEST_REPOS_DIR/main-branch/tests/test-files'"
expected="https://github.com/gilessmart/giturl/blob/$main_branch_hash/tests/test-files"
test "$description" "$command" "$expected" || ((++failed_tests))

description="With nested file path"
command="'$GITURL_PATH' '$TEST_REPOS_DIR/main-branch/tests/test-files/example.txt'"
expected="https://github.com/gilessmart/giturl/blob/$main_branch_hash/tests/test-files/example.txt"
test "$description" "$command" "$expected" || ((++failed_tests))

description="With line number option"
command="'$GITURL_PATH' -l 5 '$TEST_REPOS_DIR/main-branch/tests/test-files/example.txt'"
expected="https://github.com/gilessmart/giturl/blob/$main_branch_hash/tests/test-files/example.txt#L5"
test "$description" "$command" "$expected" || ((++failed_tests))
...
expected="https://github.com/gilessmart/giturl/blob/$main_branch_hash/tests/test-files/example-_%3D%2B%2C.%40%C2%AC%C2%A3.txt"
test "$description" "$command" "$expected" || ((++failed_tests))
